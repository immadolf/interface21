...
public abstract class AbstractBeanFactory implements HierarchicalBeanFactory {
     ...
     * 用于取消引用FactoryBean并将其与工厂创建的bean区分开。
     * 例如，如果名为myEjb的bean是工厂，则获取＆myEjb将返回工厂，而不是工厂返回的实例。
     */
     ...
     * 子类通常需要实现缓存，因为这个方法在每次请求一个bean都会被调用.
     *
     * @param beanName name of the bean to find a definition for 需要查询定义的bean的名称
     * @return the BeanDefinition for this prototype name. Must never return null.给定名称的bean的定义，不能返回null.
